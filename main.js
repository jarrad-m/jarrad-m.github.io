...
var ParticleEngine = (function() {
	'use strict';
		...
		this.lights = [{ellipseWidth:400, ellipseHeight:100, alpha:0.6, offsetX:0, offsetY:0, color:"#bf3987"}, 
						...
						{ellipseWidth:100, ellipseHeight:80, alpha:0.2, offsetX:80, offsetY:-50, color:"#bf3987"}];
		...
		function drawParticles(){
			...
			for (var i = 0, len = _ParticleEngine.particleSettings.length; i < len; i++) {
				...
				for (var s = 0; s < ball.num; s++ )
				{
					...
					if(ball.fill){
						circle.graphics.beginLinearGradientFill(["#bf3987", "#0b0b42"], [0, 1], 0, 0, 100, 100).drawRect(0, 0, 400, 400) ;
						...
						circle.graphics.beginStroke(ball.color).setStrokeStyle(0).drawCircle(0, 0, 0);
					}
					circle.rotation = 45;
					circle.alpha = range(0.5, 0.9);
		...
		{
			var scale = range(0.1, 1);
			var xpos = range(ball.initX - ball.distance, ball.initX + ball.distance);
			var ypos = range(ball.initY - ball.distance, ball.initY + ball.distance);
			var speed = ball.speed;
			TweenMax.to(ball, speed, {scaleX:scale, scaleY:scale, x:xpos, y:ypos, onComplete:animateBall, onCompleteParams:[ball], ease:Cubic.Power0});	
	...
	ParticleEngine.prototype.resize = function()
	{
		...
		for (var j = 0, len = this.lights.length; j < len; j++) {
			this.lights[j].elem.initY = this.totalHeight;
			this.lights[j].elem.initX =this.totalWidth;
